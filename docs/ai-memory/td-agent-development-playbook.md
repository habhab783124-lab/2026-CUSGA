...
- 如果任务涉及任何视觉字段、UI 文本样式、字号、颜色、缩放、图片、布局或场景表现：
  - 默认先假设 `Scene` 视图作者化结果是权威来源
  - 不要在运行时脚本里把这些字段强制写回自己的旧默认值
  - 收尾时优先检查一次 `Play` 视图是否与 `Scene` 视图保持一致
...
4. `路径校验`
5. `道路生成`
6. 如果同时改了 `BuildZone / PlacementBlocker / PlacementGrid`，执行 `放置网格工具 > Bake 当前场景静态遮罩`
7. `道路美术铺设工具`
8. `TowerDefenseValidationRunner`
...
- 对视觉表现相关开发，默认目标不是“运行时脚本接管一切”，而是“用户在 Scene 里改什么，Play 里就应看到什么”。
